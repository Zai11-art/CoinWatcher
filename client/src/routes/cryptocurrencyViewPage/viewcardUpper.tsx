...
import { IoBarcodeOutline, IoGlobeOutline, IoInformationCircleOutline, IoLayersOutline, IoLogoGithub, IoLogoReddit, IoPeopleOutline, IoPersonOutline, IoStarHalfOutline } from "react-icons/io5";
  ...
  return (
    <div
      ...
      <div
        ...
        lg:p-[3%] xl:h-[350px] xl:w-[1300px] xl:p-[2%]`}></div>
      ...
      <div
        ...
        <div className="flex flex-col justify-center md:w-[100%] lg:w-[430px]">
          ...
          <div className="mt-2 flex flex-col font-semibold">
            ...
            <div className="mt-1 flex flex-row">
              {[
                {link: response.links.homepage[0], icon: <IoGlobeOutline/>, label: "Website"},
                {link: response.links.official_forum_url[0], icon: <IoPeopleOutline/>, label: "Forum"},
                {link: response.links.subreddit_url, icon: <IoLogoReddit/>, label: "Subreddit"},
                {link: response.links.repos_url.github[0], icon: <IoLogoGithub/>, label: "Github"},
              ].map((data) => 
              <a
                href={data.link}
                key={data.label}
            ...
            >
              {[
                {content: "$" + response.market_data.high_24h.usd.toFixed(2), label: "24hr-High"},
                {content: "$" + response.market_data.low_24h.usd.toFixed(2), label: "24hr-Low"},
              ].map((data) =>
                ...
                <h1
                  className={`font-bold md:text-lg lg:text-[16px] xl:text-lg p-1 rounded-lg ${
                    response.market_data.price_change_24h > 0
                      ? `${mode === "light" ? "text-[#28a733] bg-slate-300/80 border-blue-200/60 border-[2px]" 
                      : "text-[#2ae937] bg-slate-900/80 border-cyan-200"}`
                      : `${mode === "light" ? "text-[#d82e2e] bg-slate-300/80 border-blue-200/60 border-[2px]" 
        ...
        ${mode === "light" ? "newscard-filter-light shadow-blue-900 text-slate-900 font-semibold" 
        : "bg-[#03111a] shadow-cyan-700 text-white font-semibold" }`}>

           {[
            {content: "$" + response.market_data.market_cap.usd == null ? "n/a" : response.market_data.market_cap.usd.toLocaleString(), 
            label: "Market Cap"},
            {content: "$" + response.market_data.fully_diluted_valuation.usd == null ? "n/a" : response.market_data.fully_diluted_valuation.usd.toLocaleString(), 
            label: "Fully Dilluted Valuation"},
            {content: "$" + response.market_data.circulating_supply == null ? "n/a" : response.market_data.circulating_supply.toLocaleString(), 
            label: "Circulating Supply"},
            {content: response.market_data.total_supply == null ? "n/a" : response.market_data.total_supply.toLocaleString(), 
            label: "Total Supply"},
            ].map((data) =>
            <div
              className={`
        ...
        <div
          ...
          ${mode === "light" ? "newscard-filter-light border-blue-300 shadow-blue-900 text-slate-900 font-semibold text-glow"
            : " bg-[#03111a] border-[white] shadow-cyan-700 text-white font-semibold"} md:w-[100%] lg:w-[350px]`}>

              {[
                {label: "Hashing Algorithm:", icon: <IoBarcodeOutline/>, color: "text-green-400",
                content: response.hashing_algorithm == null || response.hashing_algorithm == ""? "n/a": response.hashing_algorithm},
                {label: "Community Score:", icon: <IoStarHalfOutline/>, color: "text-violet-400",
                content: response.community_score},
                {label: "Layer type:", icon: <IoLayersOutline/>, color: "text-teal-400",
              ...
              >
                <span className={`text-4xl ${data.color} md:text-5xl lg:text-5xl`}>
                  {data.icon}
                </span>
                <div>
