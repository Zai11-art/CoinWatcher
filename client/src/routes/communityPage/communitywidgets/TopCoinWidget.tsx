...
const TopCoinWidget = () => {
  ...
  return (
    <div className={`h-[100%] w-[100%]
          ${mode === "light" ? "text-glow border-[1px] border-blue-300 bg-slate-200 font-semibold text-blue-900"
            : "chart text-white"} flex flex-col items-center rounded-xl`}>
      ...
      {prices ? (
        prices.slice(0, 5).map((data) => (
          <div
            key={data.id}
            className={` ${mode === "light"? "newscard-filter-light": "cryptocard-grad"}
                z-[1] my-2
                flex
                h-[35px] w-[90%]
                flex-row
